...
Both are checked at runtime, so an OMP release that moves them makes the plugin do less rather
...
Os dois são conferidos em tempo de execução, então uma versão nova do OMP que os mude faz o
